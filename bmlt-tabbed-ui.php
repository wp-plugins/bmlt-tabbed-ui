	...
	class BMLTTabs {
		...
		function enqueue_backend_files($hook) {
			...
			if( $hook == 'settings_page_bmlt-tabbed-ui' ) {
				...
				wp_register_style( 'bmlt-tabs-admin-css', plugins_url('css/bmlt_tabs.css', __FILE__), false, '1.0.0' );
				wp_enqueue_style( 'bmlt-tabs-admin-css' );
		
				wp_register_script('bmlt-tabs-admin', plugins_url('js/bmlt_tabs_admin.js', __FILE__), array('jquery'),'1.1', false);
...
<?php
		...
		function getMap($value) {
			...
			$map .= '<tr><td><a target="_blank" href="https://www.google.com/maps/place/' . $value[latitude] . ',' . $value[longitude] . '/@' . $value[latitude] . ',' . $value[longitude] . ',1436m/data=!3m1!4b1!4m2!3m1!1s0x0:0x0">Street Map</a></td><td>Street Map</td></tr>';
